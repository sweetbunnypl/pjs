...
class gracz2(object):
    ...
    def odczyt_klawiszy(self):
        ...
        if klawiatura[pygame.K_l] and not pygame.Rect.colliderect(self.prawa_ramka, self.gr2) and not pygame.Rect.colliderect(self.hitbox_siatka_lewy, self.gr2):
            self.keys[3] = 1
        else:
            self.keys[3] = 0
            
        if klawiatura[pygame.K_j] and not pygame.Rect.colliderect(self.hitbox_siatka_prawy, self.gr2) and not pygame.Rect.colliderect(self.lewa_ramka, self.gr2):
            self.keys[2] = 1
        else:
            self.keys[2] = 0
