...
class Game(object):
    ...
    def __init__(self):
        ...
        self.wysokosc_siatki = HEIGHT*0.5
        ...
        while True:
            ...
            while self.czas > 1/self.max_fps:
                ...
                self.dziwna_wartosc = (self.x_gracz1+(self.szerokosc_graczy/2)-(self.x_pilki+self.promien_pilki))
                self.dziwna_wartosc2 = (self.x_gracz2+(self.szerokosc_graczy/2)-(self.x_pilki+self.promien_pilki))
                text = font.render("%d///%d///%f////GRACZ 1: x:%d y:%d GRACZ 2: x:%d y:%d" % (self.dziwna_wartosc, self.dziwna_wartosc2, self.dx_pilki,self.x_gracz1, self.y_gracz1, self.x_gracz2, self.y_gracz2), True, WHITE, BLACK)
    ...
    def ruch_pilki(self):
        ...
        if pygame.Rect.colliderect(self.podloga, self.pilka):
            self.dy_pilki = -60
